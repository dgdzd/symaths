...
namespace sym {
	...
	private:
		...
		const detail::mathexpr_node* parse_mathexpr_general_func(context_table_t* ctx, uint32_t builtin_id);
		const detail::statement_node* parse_statement_func(context_table_t* ctx, uint32_t builtin_id);
