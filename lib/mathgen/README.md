...
Constants from all trees are mutated each generation with `mutateConstants` which applies a gauss offset.
...
There is a complex CMA-ES librairy inside mathgen librairy, that can be used to other purposes.
...
For harder problems, MathGen supports an Island Model: multiple independent populations (isles) evolve in parallel and periodically exchange individuals (migration). This prevents too fast convergence and allows different operator sets to let each isle be specialized. Island Model features a [Hall of Fame](#hall-of-fame) for the main group and [Backup](#backup) for every group. Usage of [convergence indicators](#convergence-indicators) is recommended.
...
The Hall of Fame is a universal entity where all best individuals get copied, it contains:
- a copy of all-time best trees
- the generation at which they were discovered
...
- a standard deviation tolerance `tol_std`
