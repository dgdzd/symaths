...
void ModelManager::fit(size_t generations, size_t maxPop, size_t eliteSize, size_t newbornSize, CMAESConfig cfg, bool debug, unsigned int timeoutSeconds,
    ...
    for (size_t gen = 0; gen < generations; gen++) {
        ...
        parallelFor(eliteSize, [&](size_t from, size_t to) {
            for (size_t i = from; i < to; i++)
                optimizeConstants(population[i].get(), X, Y, cfg);
        });
