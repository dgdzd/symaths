...
struct CMAESParams {
    ...
    CMAESParams(size_t n_, const CMAESConfig& cfg) : n(n_) {
        if (n == 0)
        ...
        //sigma
        sigma0 = cfg.sigma0;
        cs = (mu_eff + 2.0) / (static_cast<double>(n) + mu_eff + 5.0);
        ds = 1.0 + cs + 2.0 * std::max(0.0, std::sqrt((mu_eff - 1.0) / (static_cast<double>(n) + 1.0)) - 1.0);
        chiN = std::sqrt(static_cast<double>(n)) * (1.0 - 1.0 / (4.0 * static_cast<double>(n)) + 1.0 / (21.0 * static_cast<double>(n) * static_cast<double>(n)));

        //covariance
        cc  = (4.0 + mu_eff / static_cast<double>(n)) / (static_cast<double>(n) + 4.0 + 2.0 * mu_eff / static_cast<double>(n));
        c1  = 2.0 / (std::pow(static_cast<double>(n) + 1.3, 2.0) + mu_eff);
        cmu = std::min(1.0 - c1, 2.0 * (mu_eff - 2.0 + 1.0 / mu_eff) / (std::pow(static_cast<double>(n) + 2.0, 2.0) + mu_eff));
        ...
        //eigen_freq
        eigen_every = std::max(1, static_cast<int>(std::floor(1.0 / (c1 + cmu) / static_cast<double>(n) / 10.0)));
    }

    void log() const {
        printf("n=%d  lambda=%d  mu=%d  mu_eff=%.3f\n", static_cast<int>(n), lambda, mu, mu_eff);
